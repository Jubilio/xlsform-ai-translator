...
interface MicrosoftTranslation { translations?: Array<{ text: string; to: string }> }
export interface MicrosoftCredentials { apiKey?: string; region?: string }

export class MicrosoftProvider implements TranslationProvider {
  constructor(private readonly credentials: MicrosoftCredentials = {}) {}

  async translate(input: ProviderInput): Promise<ProviderOutput> {
    const key = this.credentials.apiKey || process.env.AZURE_TRANSLATOR_KEY;
    ...
    return { provider: "microsoft", translations: input.items.map((item, index) => ({ id: item.id, text: response[index]?.translations?.[0]?.text || item.text })) };
  }
}
